...
- **Params**: 
    ...
    - `tags` (list[string], optional): Filter by **Tag ID** (Precise JSON match). (e.g., `tags=4&tags=19`).
    - `tag_mode` (string, optional): Logic for `tags` list. `AND` (default) or `OR`.
    - `tag_groups` (string, optional): **Advanced**. JSON string for groups logic: `(G1) OR (G2)`. Example: `[[4, 19], [105]]` means `(Tag4 AND Tag19) OR Tag105`.
    - `exclude_tags` (list[string], optional): Filter by **Excluding** Tag ID. (e.g., `exclude_tags=60`).
...
- **Example (Advanced)**:
    - *Query 1*: Engine = RenPy, Tag = 4 (3DCG) OR 19 (Unity), Status = Ongoing
    - *URL 1*: `/games/search?engine=14&status=1&tags=4&tags=19&tag_mode=OR`
    - *Query 2 (Complex)*: (Tag 4 AND Tag 5) OR (Tag 105)
    - *URL 2*: `/games/search?tag_groups=%5B%5B4%2C5%5D%2C%5B105%5D%5D`
